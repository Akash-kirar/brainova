...
const oldLabelLogic = `{isCurrent ? (totalXP === 0 && node.id === 1 ? '0 / 100 XP' : \`\${totalXP - (parseInt(node.xp) || 0)} / \${parseInt(node.maxXP || node.xp) - (parseInt(node.xp) || 0)} XP\`) : node.status === 'completed' ? '' : \`\${node.xp} XP\`}`;
const newLabelLogic = `{isCurrent || (isLocked && node.id === 1 && totalXP < 100) ? (node.id === 1 && totalXP < 100 ? \`\${totalXP} / 100 XP\` : \`\${totalXP - (parseInt(node.xp) || 0)} / \${parseInt(node.maxXP || node.xp) - (parseInt(node.xp) || 0)} XP\`) : node.status === 'completed' ? '' : \`\${node.xp} XP\`}`;
...
console.log('App.tsx map repatched.');
