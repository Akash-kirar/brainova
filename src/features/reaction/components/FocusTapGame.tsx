...
export default function FocusTapGame({ onBack, onGameComplete, difficulty = 'easy' }: FocusTapGameProps) {
  ...
  const spawnTargets = useCallback(() => {
    ...
    for (let i = 0; i < targetCount; i++) {
      // Keep away from edges (using percentages now)
      const padding = 15;
  ...
  useEffect(() => {
    if (gameState === 'playing') {
      const timer = setTimeout(spawnTargets, 100);
